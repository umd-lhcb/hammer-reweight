...
double BToDtaunu::Compute(double q2, double thetaL, int isCLN, double ml) {
  ...
  double fplus, fminus;
  if (isCLN)
    ComputeCLN(q2, fplus, fminus);
  else
    ComputeISGW2(q2, fplus, fminus);

  return Gamma_q2tL(q2, thetaL, fplus, fminus, ml);
}

// New physics from Tanaka 2010
double BToDtaunu::Gamma_q2tL(double q2, double thetaL, double fplus,
                             double fminus, double ml) {
  double hl, hs, hsl, h0, ht;
  double ml2 = ml * ml;
  double pD  = _mB * _mB * _mB * _mB + _mD * _mD * _mD * _mD + q2 * q2 -
              2. * _mB * _mB * _mD * _mD - 2. * _mB * _mB * q2 -
              2. * _mD * _mD * q2;
  if (pD < 0)
    pD = 0;
  else
  ...
  ht = ((_mB * _mB - _mD * _mD) * fplus + q2 * fminus) / sqrt(q2) *
       (1 + _gSR * q2 / (mb_quark - mc_quark));
  hl        = h0 * h0;
  hs        = 3. * ht * ht;
  hsl       = ht * h0;
  double LH = 2. / 3 * (q2 - ml2) *
              (0.75 * sin(thetaL) * sin(thetaL) * hl +
               (ml2 / (2. * q2)) * (1.5 * cos(thetaL) * cos(thetaL) * hl +
                                    0.5 * hs + 3.0 * cos(thetaL) * hsl));

  // cout<<"h0 "<<h0<<", ht "<<ht<<", NP factor
  // "<<(1+_gSR*q2/(mb_quark-mc_quark))<<", LH "<<LH<<endl;
  return GF * GF / pow(2 * PI, 3) * Vcb * Vcb * (q2 - ml2) * pD /
         (8 * _mB * _mB * q2) * LH;
}
...
void BToDtaunu::ComputeCLN(double q2, double &fplus, double &fminus) {
  ...
  double V1 = V11 * (1. - 8. * _rho2 * z + (51. * _rho2 - 10.) * z * z -
                     (252. * _rho2 - 84.) * z * z * z);
...
void BToDtaunu::ComputeISGW2(double q2, double &fplus, double &fminus) {
  ...
  double rfppfm =
      cji * (1.0 + betaji_fppfm * EvtGetas(msq, sqrt(msb * msq)) / PI);
  double rfpmfm =
      cji * (1.0 + betaji_fpmfm * EvtGetas(msq, sqrt(msb * msq)) / PI);
  double f3fppfm = f3 * pow((mbb / mtb), -0.5) * pow((mbx / mtx), 0.5);
  double f3fpmfm = f3 * pow((mbb / mtb), 0.5) * pow((mbx / mtx), -0.5);
  double fppfm   = f3fppfm * rfppfm *
                 (2.0 - ((mtx / msq) *
                         (1 - ((msd * msq * bb2) / (2.0 * mup * mtx * bbx2)))));
  double fpmfm = f3fpmfm * rfpmfm * (mtb / msq) *
                 (1 - ((msd * msq * bb2) / (2.0 * mup * mtx * bbx2)));
...
double BToDtaunu::Normalization(double ml) {
  double CoefEBm[3][3]   = {{0.0534102, -8.07824e-11, 2.61002e-13},
                          {-0.0318507, 4.15612e-11, -1.00026e-13},
                          {0.00499707, -6.00037e-12, 1.17653e-14}};
  double CoefMuBm[3][3]  = {{0.0530911, -3.42366e-06, 1.11261e-08},
                           {-0.0315869, 1.758e-06, -4.26344e-09},
                           {0.00494503, -2.53443e-07, 4.95416e-10}};
  double CoefTauBm[3][3] = {{0.0113728, -0.000140859, 8.32051e-07},
                            {-0.00416981, 4.36796e-05, -2.25079e-07},
                            {0.000428858, -4.05144e-06, 1.89768e-08}};
  double CoefEB0[3][3]   = {{0.0494652, -8.13116e-11, 2.47308e-13},
                          {-0.0293821, 4.30308e-11, -9.51478e-14},
                          {0.00459203, -6.37326e-12, 1.06442e-14}};
  double CoefMuB0[3][3]  = {{0.0491692, -3.30057e-06, 1.05176e-08},
                           {-0.0291383, 1.71053e-06, -4.04567e-09},
                           {0.00454409, -2.48521e-07, 4.71801e-10}};
  double CoefTauB0[3][3] = {{0.0104972, -0.000130837, 7.75248e-07},
                            {-0.0038271, 4.0385e-05, -2.08983e-07},
...
// Simpson integration
double BToDtaunu::IntRate(double minX, double maxX, int isCLN, double thetaL,
  ...
  if (thetaL < -50)
    ...
    Fmin = Compute(x, thetaL, isCLN, ml);
  for (int step = 0; step < nPoints; step++) {
    if (thetaL < -50)
      Fval = Compute(x + dx / 2, isCLN, ml);
    else
      Fval = Compute(x + dx / 2, thetaL, isCLN, ml);
...
void BToDtaunu::Polynomial(double ml) {
  ...
  F[0]       = Rate(1, ml);
  _Delta     = 0;
  F[1]       = Rate(1, ml);  // Redundant, I know. For symmetry!
  _Delta     = 1;
  F[2]       = Rate(1, ml);
  Coef[0][0] = F[1];
  Coef[0][1] = (F[2] - F[0]) / 2;
  ...
  _rho2       = 1;
  _Delta      = 1;
  double F11  = Rate(1, ml) - SubPoly(Coef);
  _rho2       = -1;
  _Delta      = 1;
  double Fm11 = Rate(1, ml) - SubPoly(Coef);
  _rho2       = 1;
  _Delta      = -1;
  double F1m1 = Rate(1, ml) - SubPoly(Coef);
  _rho2       = 2;
  _Delta      = -1;
  double F2m1 = Rate(1, ml) - SubPoly(Coef);
  ...
  _rho2  = 1.18;
  _Delta = 1;  // Check that the calculation is correct
...
double BToDtaunu::SubPoly(double C[3][3]) {
  return C[0][0] + C[1][0] * _rho2 + C[2][0] * _rho2 * _rho2 +
         C[0][1] * _Delta + C[0][2] * _Delta * _Delta;
}

void BToDtaunu::SettBmH(double tBmH) { _gSR = -mb_quark * pow(tBmH, 2); }
