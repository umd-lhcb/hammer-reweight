// Author: Yipeng Sun
// Last Change: Tue Aug 24, 2021 at 07:26 PM +0200
...
RwRate reweight(TFile* input_ntp, TFile* output_ntp, TString tree,
  ...
  // ff_calc
  auto calc_BDst = BToDstaunu(1.122, 1.270, 0.852, 1.15);
  ...
  Double_t w_ff_calc_out;
  output_tree->Branch("w_ff_calc", &w_ff_calc_out);
  ...
  bool          is_Dst            = false;
  bool          is_D              = false;
  double        ff_calc_isgw2     = 1.;
  double        ff_calc_cln       = 1.;
  ...
  while (reader.Next()) {
    ...
    if (find_in(LEGAL_B_MESON_IDS, TMath::Abs(*b_id)) && *q2 > q2_min &&
      ...
      if (D_ok) {
        if (TMath::Abs(D_cands[D_lbl]) == 421) is_D = true;
        if (TMath::Abs(D_cands[D_lbl]) == 413) is_Dst = true;
        ...
        if (proc_id != 0 && find(part_m2_ok.begin(), part_m2_ok.end(), false) ==
          ...
          if (is_Dst) {
            ff_calc_isgw2 = calc_BDst.Compute(q2_true_out, false, TAU_MASS);
            ff_calc_cln   = calc_BDst.Compute(q2_true_out, true, TAU_MASS);
          }

          if (!isnan(ff_out) && !isinf(ff_out)) {
            w_ff_out = ff_out;
            // w_ff_calc_out = ff_calc_cln / ff_calc_isgw2;
            num_of_evt_ham_ok += 1;
            ham_ok = true;
          }
        }
